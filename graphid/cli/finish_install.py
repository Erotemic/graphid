...
class FinishInstallCLI(scfg.DataConfig):
    ...
    def main(FinishInstallCLI, cmdline=1, **kwargs):
        ...
        if install_sections['graphviz']:
            if is_debian():
                pkgname_list = ['graphviz', 'libgraphviz-dev']
                debian_ensure_packages(pkgname_list)
            else:
                raise NotImplementedError

        # New experimental logic
        if install_sections['pygraphviz']:
...
def debian_ensure_packages(pkgname_list):
    pkgname_list_str = ' '.join(pkgname_list)
...
def is_debian():
    try:
        ub.cmd('type apt', shell=True, check=True)
    except Exception:
        return False
    return True


def debian_has_package(pkgname):
    out = ub.cmd(f'dpkg -l "{pkgname}" | grep "^ii *{pkgname}" > /dev/null', shell=True)
    return out['ret'] == 0
...
if __name__ == '__main__':
    ...
    CommandLine:
        python ~/code/graphid/graphid/cli/finish_install.py
    """
    main()
