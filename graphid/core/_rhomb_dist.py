...
def RhombicuboctahedronDistanceDemo():
    ...
    def rhombicuboctahedro_faces():
        ...
        for i in range(1, len(face_axes) + 1):
            for axes in list(it.combinations(face_axes, i)):
                for combo in it.product(*axes):
                    sortx = ub.argsort(ub.take(ordering, combo))
                    face = tuple(ub.take(combo, sortx))
                    yield face
    ...
    edges = []
    for face1, face2 in it.combinations(faces, 2):
        set1 = set(face1)
        set2 = set(face2)
        if len(set1.intersection(set2)) > 0:
            diff1 = set1.difference(set2)
            diff2 = set2.difference(set1)
            sortx1 = ub.argsort(ub.take(ordering, diff1))
            sortx2 = ub.argsort(ub.take(ordering, diff2))
    ...
    # for i in range(0, 5):
    #     print(ut.urepr({k: v for k, v in z.items() if v == i}, si=True))
    # i = None
    # print(ut.urepr({k: v for k, v in z.items() if v == i}, si=True))

    # z = ut.sort_dict(z, 'vals')
    # print(ut.urepr(z, nl=2, si=True))
...
if __name__ == '__main__':
    """
    CommandLine:
        python -m graphid.core._rhomb_dist all
    """
    import xdoctest
